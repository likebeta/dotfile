...
set fileencodings=utf-8,ucs-bom,utf-8-bom,cp936,big5,gb18030,ucs
...
au BufWinEnter * let w:m2=matchadd('OverLength', '\%>' . 100 . 'v.\+', -1)
...
function! ViewComments(...)
    ...
    execute "make -s http://10.6.193.143:8080/" . join(a:000) . "/comments"
    let &makeprg=old_makeprg
endfunction
...
function InsertPythonComment()
    ...
    call setline('.', '')
    normal o
    call setline('.', '#  Author: 易思龙 <ixxoo.me@gmail.com>')
    normal o
    call setline('.', '#  Create: '.strftime("%Y-%m-%d"))
    call cursor(7, 0)
endfunction
...
function InsertCommentWhenOpen()
    if a:lastline == 1 && !getline('.') && &filetype == 'python'
        call InsertPythonComment()
    elseif a:lastline == 1 && !getline('.') && &filetype == 'sh'
        call InsertShellComment()
    end
endfunction
au FileType python :%call InsertCommentWhenOpen()
au FileType sh :%call InsertCommentWhenOpen()
